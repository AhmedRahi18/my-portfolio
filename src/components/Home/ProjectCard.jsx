  ...
  return (
    ...
    >
      <img
        src={image}
        alt="Image"
        className={`w-96 mx-auto md:mx-0 md:w-[500px] md:transition-transform duration-300 ${
          isHovered ? 'md:scale-110 brightness-50' : ''
        }`}
      />
      {isHovered && (
        <div className="absolute top-0  left-0 w-full h-full">
          <p className="text-white text-center mt-5 font-bold md:mb-3 text-xl md:text-2xl md:me-20">{name}</p>
          <p className="text-white md:me-20 text-xs md:text-sm md:ms-5">{details}</p>
          <div className="flex md:mt-3 md:ms-20">
